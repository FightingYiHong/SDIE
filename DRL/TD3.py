...
import numpy as np
from DRL.networks import ActorNetwork, CriticNetwork
...
class TD3:
    def __init__(self, alpha, beta, state_dim, action_dim, actor_fc1_dim, actor_fc2_dim,
        ...
        self.actor = ActorNetwork(alpha=alpha, state_dim=state_dim, action_dim=action_dim,
                                  fc1_dim=actor_fc1_dim, fc2_dim=actor_fc2_dim).to(device)
        self.critic1 = CriticNetwork(beta=beta, state_dim=state_dim, action_dim=action_dim,
                                     fc1_dim=critic_fc1_dim, fc2_dim=critic_fc2_dim).to(device)
    ...
    def update_network_parameters(self, tau=None):
        if tau is None:
            tau = self.tau
        print(sum(i.nelement() for i in self.actor.parameters()))
        print(sum(i.nelement() for i in self.target_actor.parameters()))
